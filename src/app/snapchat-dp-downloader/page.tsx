...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("snapchat-dp-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Snapchat DP Downloader – Save Snap Profile Pictures HD",
    description: seoData.meta?.description || "Download Snapchat profile pictures in crisp HD resolution. Save any public Snap profile avatar directly to your local device.",
    alternates: getSeoAlternates("snapchat-dp-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("snapchat-dp-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Snapchat DP Downloader",
          description: "Download high-resolution Snapchat Profile Pictures (DP) and Bitmojis online for free.",
          ratingValue: "4.9",
          reviewCount: "4200"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "DP Downloader", item: `${locale === 'en' ? '' : '/' + locale}/snapchat-dp-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "DP Downloader", href: "/snapchat-dp-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-yellow-400 text-black px-4">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90">
                View and download full HD Snapchat Bitmojis and Profile Photos anonymously.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/snapchat-dp-downloader`}
