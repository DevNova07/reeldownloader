...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("telegram-video-compressor", 'en');
  return { 
    title: seoData.meta?.title || "Telegram Video Compressor – Reduce TG Video File Size",
    description: seoData.meta?.description || "Compress large Telegram videos online. Reduce file size to pass TG upload restrictions while maintaining maximum audio/video fidelity.",
    alternates: getSeoAlternates("telegram-video-compressor", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("telegram-video-compressor", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Telegram Video Compressor",
          description: "Compress and reduce the file size of Telegram videos online without losing quality.",
          ratingValue: "4.8",
          reviewCount: "3100"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Video Compressor", item: `${locale === 'en' ? '' : '/' + locale}/telegram-video-compressor` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Video Compressor", href: "/telegram-video-compressor" }
        ]} 
      />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <TelegramPreviewMockup type="compressor" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-sky-600 text-white px-4 border-t border-sky-500">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90 text-sky-100">
                Compress bulky Telegram videos into lightweight MP4s perfect for sharing and storage.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/telegram-video-compressor`}
