...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("telegram-dp-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Telegram DP Downloader – Save TG Profile Pictures HD",
    description: seoData.meta?.description || "Download Telegram profile pictures and group avatars in full resolution. Save TG display photos anonymously via links.",
    alternates: getSeoAlternates("telegram-dp-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("telegram-dp-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Telegram DP Downloader",
          description: "Download Telegram profile pictures and avatars in full HD resolution.",
          ratingValue: "4.9",
          reviewCount: "5800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "DP Downloader", item: `${locale === 'en' ? '' : '/' + locale}/telegram-dp-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "DP Downloader", href: "/telegram-dp-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-sky-600 text-white px-4 border-t border-sky-500">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8 leading-tight">
                Save TG Avatars Instantly!
            </h2>
            <p className="text-xl font-bold mb-12 opacity-90 text-sky-100">
                View and download high-resolution Telegram profile pictures securely with our web tool.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/telegram-dp-downloader`}
