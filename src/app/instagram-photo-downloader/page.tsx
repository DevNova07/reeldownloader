...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("instagram-photo-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Download Instagram Photos HD & Full Res – SavClip",
    description: seoData.meta?.description || "Download Instagram photos and images in full HD resolution. Save original high-resolution pictures from any IG post for free.",
    alternates: getSeoAlternates("instagram-photo-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("instagram-photo-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Instagram Photo Downloader",
          description: "Download high-resolution Instagram photos and galleries for free.",
          ratingValue: "4.9",
          reviewCount: "7800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Photo Downloader", item: `${locale === 'en' ? '' : '/' + locale}/instagram-photo-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Photo Downloader", href: "/instagram-photo-downloader" }
        ]} 
      />
      ...
      <FeaturesSection platform="Instagram Photo" />
      ...
      <FAQSection platform="Instagram Photo" items={faqs} />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <InstagramPreviewMockup type="photo" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-neutral-50 dark:bg-neutral-900/50 px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "How to Save High-Res IG Photos", desc: "Get the maximum pixels for your favorite shots." },
              { title: "Managing Instagram Photo Galleries", desc: "Tips for organizing and archiving your visual history." },
              { title: "The Best Formats for Social Media", desc: "JPEG vs PNG: Which one should you choose?" }
            ].map((blog, i) => (
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-sky-500/30 transition-all">
                ...
                <Link href={`${locale === 'en' ? '/' : '/' + locale}`} className="text-xs font-black uppercase tracking-widest text-sky-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
