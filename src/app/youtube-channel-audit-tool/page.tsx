...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("youtube-channel-audit-tool", 'en');
  return { 
    title: seoData.meta?.title || "YouTube Channel Audit Tool – Analyze YT Channels SEO & Stats",
    description: seoData.meta?.description || "Audit any YouTube channel",
    alternates: getSeoAlternates("youtube-channel-audit-tool", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("youtube-channel-audit-tool", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip YouTube Channel Audit Tool",
          description: "Audit YouTube channels for SEO tags, metadata, and stats online for free.",
          ratingValue: "4.8",
          reviewCount: "2100"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Channel Audit", item: `${locale === 'en' ? '' : '/' + locale}/youtube-channel-audit-tool` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Channel Audit", href: "/youtube-channel-audit-tool" }
        ]} 
      />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <YoutubePreviewMockup type="audit" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-red-600 text-white px-4">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-8 leading-tight">
                Unlock Channel Secrets!
            </h2>
            <p className="text-xl font-bold mb-12 opacity-90">
                Discover the SEO tags and growth strategies of top creators with the best online audit tool.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/youtube-channel-audit-tool`}
