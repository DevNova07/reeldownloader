...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("facebook-group-video-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Facebook Group Video Downloader – Save Group Videos Online | SavClip",
    description: seoData.meta?.description || "Easily save videos from public Facebook groups. Use SavClip to backup group video content directly to your phone or PC in HD.",
    alternates: getSeoAlternates("facebook-group-video-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("facebook-group-video-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Facebook Group Video Downloader",
          description: "Download Facebook group videos in HD quality for free online.",
          ratingValue: "4.8",
          reviewCount: "2900"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Group Downloader", item: `${locale === 'en' ? '' : '/' + locale}/facebook-group-video-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Group Downloader", href: "/facebook-group-video-downloader" }
        ]} 
      />
      ...
      <FeaturesSection platform="Facebook Group" />
      ...
      <FAQSection platform="Facebook Group" items={faqs} />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <FacebookPreviewMockup type="group" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-blue-600 text-white px-4">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-8 leading-tight">
                Archive Your Communities!
            </h2>
            <p className="text-xl font-bold mb-12 opacity-90">
                Save the best moments from your Facebook groups in HD with the world's most secure downloader.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/facebook-group-video-downloader`}
