...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-media-downloader", 'en');
  return { 
    title: seoData.meta?.title || "X Media Downloader – Download Twitter Photos & Videos",
    description: seoData.meta?.description || "Download photos, videos, and multi-image attachments from X (Twitter) posts in high resolution using a single URL.",
    alternates: getSeoAlternates("x-media-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("x-media-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip X Media Downloader",
          description: "Download photos, videos, and GIFs from X (Twitter) in original high quality for free.",
          ratingValue: "4.9",
          reviewCount: "8800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Media Downloader", item: `${locale === 'en' ? '' : '/' + locale}/x-media-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Media Downloader", href: "/x-media-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-black text-white px-4 border-t border-neutral-800">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90 text-neutral-400">
                Download original high-quality photos, videos, and mixed media from Twitter effortlessly.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/x-media-downloader`}
