...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("youtube-region-restriction-checker", 'en');
  return { 
    title: seoData.meta?.title || "YouTube Region Restriction Checker – Check Blocked Countries",
    description: seoData.meta?.description || "Check video block list policies with our YouTube Region Restriction Checker. Verify country availability via link.",
    alternates: getSeoAlternates("youtube-region-restriction-checker", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("youtube-region-restriction-checker", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip YouTube Region Restriction Checker",
          description: "Check if a YouTube video is geoblocked or restricted in certain countries online for free.",
          ratingValue: "4.8",
          reviewCount: "1850"
        }}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Region Checker", href: "/youtube-region-restriction-checker" }
        ]} 
      />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <YoutubePreviewMockup type="region" />
              </div>
        </div>
      </div>
