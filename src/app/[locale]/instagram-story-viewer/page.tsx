...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { 
    ...
    description: "View and download Instagram stories anonymously in high resolution. Watch IG stories privately without leaving a view trace or login.",
    ...
    alternates: getSeoAlternates("instagram-story-viewer", params.locale),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Story Viewer", item: `/${locale}/instagram-story-viewer` }
        ]}
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-purple-500/30 transition-all">
                ...
                <Link href={`/${locale}`} className="text-xs font-black uppercase tracking-widest text-purple-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
