...
export default function TikTokMusicPage() {
  ...
  const handleSearch = async (url: string) => {
    ...
    const searchPromise = async () => {
      const response = await fetch("/api/download", {
        ...
        body: JSON.stringify({ url, type: "audio", platform: "tiktok" }),
      })
  ...
  return (
    <div className="flex flex-col">
      ...
      <StructuredData
        type="HowTo"
        data={{
          name: tiktokDict.music.howTo.name,
          description: tiktokDict.music.howTo.description,
          steps: tiktokDict.music.howTo.steps
        }}
      />
      ...
      <VisualGuide 
        platformName={`${dict.categories.tiktok} ${dict.tabs.music}`}
        accentColor="text-pink-600"
        bgAccentColor="bg-pink-600"
        Icon={MusicIcon}
        steps={[
          { title: dict.guide.steps[0].title.replace('{platform}', dict.categories.tiktok), desc: tiktokDict.music.howTo.steps[0], image: "/images/how-to/step1.webp" },
          { title: dict.guide.steps[1].title.replace('{platform}', dict.categories.tiktok), desc: tiktokDict.music.howTo.steps[1] + " " + tiktokDict.music.howTo.steps[2], image: "/images/how-to/step2.webp" },
          { title: dict.guide.steps[2].title.replace('{platform}', dict.categories.tiktok), desc: tiktokDict.music.howTo.steps[3], image: "/images/how-to/step3.webp" },
        ]}
      />
