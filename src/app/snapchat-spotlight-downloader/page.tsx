...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("snapchat-spotlight-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Snapchat Spotlight Downloader: Save Snaps HD",
    description: seoData.meta?.description || "Download Snapchat Spotlight videos in HD without any watermark. Save viral public short clips from Snap creators instantly.",
    alternates: getSeoAlternates("snapchat-spotlight-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("snapchat-spotlight-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Snapchat Spotlight Downloader",
          description: "Download viral Snapchat Spotlight videos in HD without watermarks.",
          ratingValue: "4.9",
          reviewCount: "9200"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Spotlight Downloader", item: `${locale === 'en' ? '' : '/' + locale}/snapchat-spotlight-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Spotlight Downloader", href: "/snapchat-spotlight-downloader" }
        ]} 
      />
      ...
      <FeaturesSection platform="Snapchat Spotlight" />
      ...
      <FAQSection platform="Snapchat Spotlight" items={faqs} />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <SnapchatPreviewMockup type="spotlight" />
              </div>
        </div>
      </div>
