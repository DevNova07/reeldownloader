...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-gif-downloader", 'en');
  return { 
    title: seoData.meta?.title || "X GIF Downloader – Save Twitter GIFs as MP4",
    description: seoData.meta?.description || "Save animations from X (Twitter) as standard loopable MP4 files. Download public X GIFs directly to your device gallery.",
    alternates: getSeoAlternates("x-gif-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("x-gif-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip X GIF Downloader",
          description: "Download GIFs from X (Twitter) as high-quality MP4 files for free.",
          ratingValue: "4.8",
          reviewCount: "9100"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "GIF Downloader", item: `${locale === 'en' ? '' : '/' + locale}/x-gif-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "GIF Downloader", href: "/x-gif-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-black text-white px-4 border-t border-neutral-800">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8 leading-tight">
                Save X GIFs Instantly!
            </h2>
            <p className="text-xl font-bold mb-12 opacity-90 text-neutral-400">
                Download looping media and animated GIFs from Twitter securely with our web tool.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/x-gif-downloader`}
