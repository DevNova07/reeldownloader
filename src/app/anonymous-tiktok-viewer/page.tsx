...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("anonymous-tiktok-viewer", 'en');
  return { 
    title: seoData.meta?.title || "Anonymous TikTok Viewer – View TikTok Secretly",
    description: seoData.meta?.description || "View and explore TikTok videos secretly. SavClip lets you watch trending TikTok content anonymously without an account or login.",
    alternates: getSeoAlternates("anonymous-tiktok-viewer", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("anonymous-tiktok-viewer", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Anonymous TikTok Viewer",
          description: "View and download public TikTok profiles and videos secretly without an account.",
          ratingValue: "4.9",
          reviewCount: "5200"
        }}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Anonymous Viewer", href: "/anonymous-tiktok-viewer" }
        ]} 
      />
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { title: "DP Downloader", href: "/tiktok-dp-downloader" },
              ].map((tool, i) => (
              <Link 
                key={i}
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <TiktokPreviewMockup type="anonymous" />
              </div>
        </div>
      </div>
