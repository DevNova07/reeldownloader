...
import { FacebookStoryCTA } from "./FacebookStoryCTA"
...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("facebook-story-saver", 'en');
  return { 
    title: seoData.meta?.title || "Facebook Story Downloader – View FB Stories Anonymously HD",
    description: seoData.meta?.description || "Download and view Facebook stories anonymously in HD quality. Save FB status updates and stories before they disappear in 24 hours.",
    alternates: getSeoAlternates("facebook-story-saver", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("facebook-story-saver", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Facebook Story Downloader",
          description: "Download Facebook stories and view them anonymously in HD quality for free.",
          ratingValue: "4.9",
          reviewCount: "7300"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Story Saver", item: `${locale === 'en' ? '' : '/' + locale}/facebook-story-saver` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Story Saver", href: "/facebook-story-saver" }
        ]} 
      />
      ...
      <FeaturesSection platform="Facebook Story" />
      ...
      <FAQSection platform="Facebook Story" items={faqs} />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <FacebookPreviewMockup type="story" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-neutral-50 dark:bg-neutral-900/50 px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "How to View FB Stories Anonymously", desc: "The ultimate guide to staying hidden while watching." },
              { title: "Archiving Your Best FB Moments", desc: "Why you should save your own stories locally." },
              { title: "The Best FB Story Viewer Apps 2026", desc: "Why SavClip's web-based tool is the best choice." }
            ].map((blog, i) => (
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-indigo-500/30 transition-all">
                ...
                <Link href={`${locale === 'en' ? '/' : '/' + locale}`} className="text-xs font-black uppercase tracking-widest text-indigo-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
