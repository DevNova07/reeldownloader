...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("facebook-dp-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Facebook DP Downloader – Save Profile Pictures HD | SavClip",
    description: seoData.meta?.description || "Download Facebook profile pictures (DP) in full resolution and high quality. Save any FB profile photo anonymously without logging in.",
    alternates: getSeoAlternates("facebook-dp-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("facebook-dp-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Facebook DP Downloader",
          description: "Download Facebook profile pictures in HD quality for free online.",
          ratingValue: "4.9",
          reviewCount: "1800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "DP Downloader", item: `${locale === 'en' ? '' : '/' + locale}/facebook-dp-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "DP Downloader", href: "/facebook-dp-downloader" }
        ]} 
      />
      ...
      <FeaturesSection platform="Facebook DP" />
      ...
      <FAQSection platform="Facebook DP" items={faqs} />
      ...
      <section className="py-20 bg-blue-600 text-white px-4">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl md:text-6xl font-black uppercase italic tracking-tighter mb-8 leading-tight">
                HD Profile Pictures Instantly!
            </h2>
            <p className="text-xl font-bold mb-12 opacity-90">
                Never miss a detail. Download Facebook DPs in full resolution for free with SavClip.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/facebook-dp-downloader`}
