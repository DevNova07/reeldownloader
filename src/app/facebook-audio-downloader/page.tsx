...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("facebook-audio-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Facebook Audio Downloader – Convert FB Video to MP3 HD",
    description: seoData.meta?.description || "Convert Facebook videos to MP3 audio files instantly. Extract high-quality sound tracks from FB posts safely with SavClip.",
    alternates: getSeoAlternates("facebook-audio-downloader", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Audio Downloader", item: `${locale === 'en' ? '' : '/' + locale}/facebook-audio-downloader` }
        ]}
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-emerald-500/30 transition-all">
                ...
                <Link href={`${locale === 'en' ? '/' : '/' + locale}`} className="text-xs font-black uppercase tracking-widest text-emerald-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
