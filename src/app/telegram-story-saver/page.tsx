...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("telegram-story-saver", 'en');
  return { 
    title: seoData.meta?.title || "Telegram Story Saver – Download TG Stories Anonymously",
    description: seoData.meta?.description || "Download Telegram stories anonymously in high resolution. Keep temporary TG video status clips before they disappear.",
    alternates: getSeoAlternates("telegram-story-saver", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Story Saver", item: `${locale === 'en' ? '' : '/' + locale}/telegram-story-saver` }
        ]}
      />
      ...
      <section className="py-20 bg-sky-600 text-white px-4 border-t border-sky-500">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/telegram-story-saver`}
