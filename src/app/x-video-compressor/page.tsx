...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-video-compressor", 'en');
  return { 
    title: seoData.meta?.title || "X Video Compressor – Reduce Twitter Video File Size Free | SavClip",
    description: seoData.meta?.description || "Reduce video file sizes for seamless sharing on X (Twitter). Compress file assets to meet platform upload rules for free.",
    alternates: getSeoAlternates("x-video-compressor", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("x-video-compressor", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip X Video Compressor",
          description: "Compress and reduce the file size of X (Twitter) videos online without losing quality.",
          ratingValue: "4.8",
          reviewCount: "3200"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Video Compressor", item: `${locale === 'en' ? '' : '/' + locale}/x-video-compressor` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Video Compressor", href: "/x-video-compressor" }
        ]} 
      />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <XPreviewMockup type="compressor" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-black text-white px-4 border-t border-neutral-800">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90 text-neutral-400">
                Compress bulky X (Twitter) videos into lightweight MP4s perfect for sharing and storage.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/x-video-compressor`}
