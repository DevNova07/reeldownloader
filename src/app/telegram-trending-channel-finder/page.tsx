...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("telegram-trending-channel-finder", 'en');
  return { 
    title: seoData.meta?.title || "Telegram Trending Channel Finder – Discover Viral TG Groups",
    description: seoData.meta?.description || "Discover viral, trending, and top-rated Telegram groups and channels across multiple niches using our interactive finder.",
    alternates: getSeoAlternates("telegram-trending-channel-finder", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("telegram-trending-channel-finder", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Telegram Trending Channel Finder",
          description: "Identify viral and trending channels and groups on Telegram using real-time AI analysis.",
          ratingValue: "4.9",
          reviewCount: "6400"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Channel Finder", item: `${locale === 'en' ? '' : '/' + locale}/telegram-trending-channel-finder` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Channel Finder", href: "/telegram-trending-channel-finder" }
        ]} 
      />
      ...
      <section className="py-20 bg-white dark:bg-black px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { title: "Analytics Viewer", href: "/telegram-analytics-viewer" },
              { title: "Video Downloader", href: "/telegram-video-downloader" },
              { title: "Story Saver", href: "/telegram-story-saver" },
              { title: "Bio Generator", href: "/telegram-bio-generator" }
            ].map((tool, i) => (
              <Link 
                key={i}
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <TelegramPreviewMockup type="channels" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-sky-600 text-white px-4 border-t border-sky-500">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90 text-sky-100">
                Identify viral channels and emerging communities on Telegram securely with our AI tool.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/telegram-trending-channel-finder`}
