...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-analytics-viewer", 'en');
  return { 
    title: seoData.meta?.title || "X Analytics Viewer – Check Twitter Profile Stats Free",
    description: seoData.meta?.description || "Check public metrics, profile health, and analytics charts for any X (Twitter) handle absolutely free without tracking tags.",
    alternates: getSeoAlternates("x-analytics-viewer", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("x-analytics-viewer", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip X Analytics Viewer",
          description: "Analyze X (Twitter) profile statistics, engagement rates, and growth for free.",
          ratingValue: "4.9",
          reviewCount: "5800"
        }}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Analytics Viewer", href: "/x-analytics-viewer" }
        ]} 
      />
