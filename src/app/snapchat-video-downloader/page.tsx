...
import { SnapchatCTA } from "./SnapchatCTA"
...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("snapchat-video-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Snapchat Video Downloader – Save Snaps & Spotlight Videos | SavClip",
    description: seoData.meta?.description || "Download Snapchat videos to MP4 files. The ultimate free tool to extract public videos from Snapchat links safely.",
    alternates: getSeoAlternates("snapchat-video-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("snapchat-video-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Snapchat Video Downloader",
          description: "Download Snapchat videos in HD without watermarks for free.",
          ratingValue: "4.8",
          reviewCount: "8500"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Video Downloader", item: `${locale === 'en' ? '' : '/' + locale}/snapchat-video-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Video Downloader", href: "/snapchat-video-downloader" }
        ]} 
      />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <SnapchatPreviewMockup type="video" />
              </div>
        </div>
      </div>
