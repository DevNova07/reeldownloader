...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("instagram-audio-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Instagram Audio Downloader – Extract & Save IG MP3 Music | SavClip",
    description: seoData.meta?.description || "Extract and download audio tracks from Instagram reels and videos. Save IG audio files as high-quality MP3s instantly.",
    alternates: getSeoAlternates("instagram-audio-downloader", 'en'),
  }
}
