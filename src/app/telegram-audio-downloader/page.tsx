...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("telegram-audio-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Telegram Audio Downloader – Extract & Save TG MP3 Music",
    description: seoData.meta?.description || "Extract and download voice notes, music tracks, and MP3 files directly from Telegram channels and chat links.",
    alternates: getSeoAlternates("telegram-audio-downloader", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Audio Downloader", item: `${locale === 'en' ? '' : '/' + locale}/telegram-audio-downloader` }
        ]}
      />
      ...
      <section className="py-20 bg-sky-600 text-white px-4 border-t border-sky-500">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/telegram-audio-downloader`}
