...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("instagram-video-compressor", 'en');
  return { 
    title: seoData.meta?.title || "Instagram Video Compressor: Resize IG Videos",
    description: seoData.meta?.description || "Compress Instagram videos to fit upload limits perfectly. Resize and optimize clips for IG stories, reels, and posts online free.",
    alternates: getSeoAlternates("instagram-video-compressor", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("instagram-video-compressor", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Instagram Video Compressor",
          description: "Compress and resize Instagram videos online without quality loss.",
          ratingValue: "4.9",
          reviewCount: "3800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Video Compressor", item: `${locale === 'en' ? '' : '/' + locale}/instagram-video-compressor` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Video Compressor", href: "/instagram-video-compressor" }
        ]} 
      />
      ...
      <section className="py-20 bg-neutral-50 dark:bg-neutral-900/50 px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-black text-neutral-900 dark:text-white uppercase italic tracking-wider md:tracking-widest text-center mb-12 leading-tight">
            Compression & Quality Guides
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "Optimizing Video Size for Viral Reels", desc: "How to balance quality and speed for maximum reach." },
              { title: "Best Video Formats for Instagram 2026", desc: "A guide to maintaining clarity across all post types." },
              { title: "Why Video Compression is Essential", desc: "Understanding the impact of file size on engagement." }
            ].map((blog, i) => (
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-blue-500/30 transition-all">
                ...
                <Link href={`${locale === 'en' ? '/' : '/' + locale}`} className="text-xs font-black uppercase tracking-widest text-blue-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
