...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-space-downloader", 'en');
  return { 
    title: seoData.meta?.title || "X Space Downloader – Record & Save Twitter Spaces as MP3",
    description: seoData.meta?.description || "Record and download full audio sessions from completed X (Twitter) Spaces. Save long conversations as manageable MP3 tracks.",
    alternates: getSeoAlternates("x-space-downloader", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Space Downloader", item: `${locale === 'en' ? '' : '/' + locale}/x-space-downloader` }
        ]}
      />
      ...
      <section className="py-20 bg-black text-white px-4 border-t border-neutral-800">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/x-space-downloader`}
