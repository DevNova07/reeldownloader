...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("instagram-dp-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Instagram DP Downloader – Download Profile Picture Full HD | SavClip",
    description: seoData.meta?.description || "Save Instagram profile pictures in full resolution. Download any IG DP in high quality anonymously without logging into an account.",
    alternates: getSeoAlternates("instagram-dp-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("instagram-dp-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Instagram DP Downloader",
          description: "Download Instagram profile pictures in full size and HD quality.",
          ratingValue: "4.9",
          reviewCount: "4800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "DP Downloader", item: `/instagram-dp-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "DP Downloader", href: "/instagram-dp-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-neutral-50 dark:bg-neutral-900/50 px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "How to Get Full Size Instagram DPs", desc: "A guide to bypassing mobile compression for sharp icons." },
              { title: "Viewing Profiles Anonymously 2026", desc: "Why savvy users prefer SavClip for private browsing." },
              { title: "The Importance of Profile Aesthetics", desc: "How a high-quality DP defines your digital identity." }
            ].map((blog, i) => (
