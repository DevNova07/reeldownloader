...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-profile-picture-downloader", 'en');
  return { 
    title: seoData.meta?.title || "X Profile Picture Downloader – Save Twitter DP in Full HD",
    description: seoData.meta?.description || "Download X (Twitter) profile photos in original full resolution. Save public avatars safely without logging in.",
    alternates: getSeoAlternates("x-profile-picture-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("x-profile-picture-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip X Profile Picture Downloader",
          description: "Download X (Twitter) profile pictures and avatars in full HD resolution.",
          ratingValue: "4.9",
          reviewCount: "6500"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "DP Downloader", item: `${locale === 'en' ? '' : '/' + locale}/x-profile-picture-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "DP Downloader", href: "/x-profile-picture-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-white dark:bg-black px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {[
              { title: "Banner Downloader", href: "/x-banner-downloader" },
      ...
      <section className="py-20 bg-black text-white px-4 border-t border-neutral-800">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl md:text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8 leading-tight">
                Save X Avatars Instantly!
            </h2>
            <p className="text-xl font-bold mb-12 opacity-90 text-neutral-400">
                View and download high-resolution Twitter profile pictures securely with our web tool.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/x-profile-picture-downloader`}
