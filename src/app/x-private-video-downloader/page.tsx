...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("x-private-video-downloader", 'en');
  return { 
    title: seoData.meta?.title || "X Private Video Downloader – Save Locked Twitter Videos | SavClip",
    description: seoData.meta?.description || "Extract private videos from X (Twitter) accounts securely. Save exclusive visual assets for secure offline backups.",
    alternates: getSeoAlternates("x-private-video-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("x-private-video-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip X Private Video Downloader",
          description: "Download private X (Twitter) videos securely using source code extraction.",
          ratingValue: "4.8",
          reviewCount: "4400"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Private Downloader", item: `${locale === 'en' ? '' : '/' + locale}/x-private-video-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Private Downloader", href: "/x-private-video-downloader" }
        ]} 
      />
      ...
      <section className="py-20 bg-black text-white px-4 border-t border-neutral-800">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90 text-neutral-400">
                Download private Twitter videos securely without sharing your password.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/x-private-video-downloader`}
