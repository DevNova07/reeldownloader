...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("telegram-private-video-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Telegram Private Video Downloader – Save Locked TG Videos",
    description: seoData.meta?.description || "Download private videos from restrictive Telegram channels securely. Bypass local streaming delays and save content offline.",
    alternates: getSeoAlternates("telegram-private-video-downloader", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Private Downloader", item: `${locale === 'en' ? '' : '/' + locale}/telegram-private-video-downloader` }
        ]}
      />
      ...
      <section className="py-20 bg-sky-600 text-white px-4 border-t border-sky-500">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/telegram-private-video-downloader`}
