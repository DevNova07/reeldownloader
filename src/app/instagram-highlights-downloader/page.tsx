...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("instagram-highlights-downloader", 'en');
  return { 
    title: seoData.meta?.title || "Download Instagram Highlights HD & Original Quality",
    description: seoData.meta?.description || "Download Instagram highlights directly to your device. Save active or archived IG highlight stories in pristine original quality.",
    alternates: getSeoAlternates("instagram-highlights-downloader", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Highlights Downloader", item: `${locale === 'en' ? '' : '/' + locale}/instagram-highlights-downloader` }
        ]}
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-amber-500/30 transition-all">
                ...
                <Link href={`${locale === 'en' ? '/' : '/' + locale}`} className="text-xs font-black uppercase tracking-widest text-amber-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
