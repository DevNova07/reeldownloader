...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("youtube-to-mp3-converter", 'en');
  return { 
    title: seoData.meta?.title || "YouTube to MP3 Converter – Download YT MP3 320kbps",
    description: seoData.meta?.description || "Convert YouTube videos to premium 320kbps MP3 tracks. Download full music streams safely without software setup.",
    alternates: getSeoAlternates("youtube-to-mp3-converter", 'en'),
  }
}
    ...
    <>
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "MP3 Converter", item: `${locale === 'en' ? '' : '/' + locale}/youtube-to-mp3-converter` }
        ]}
      />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-rose-500/30 transition-all">
                ...
                <Link href={`${locale === 'en' ? '/' : '/' + locale}`} className="text-xs font-black uppercase tracking-widest text-rose-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
