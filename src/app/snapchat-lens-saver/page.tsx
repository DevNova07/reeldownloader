...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("snapchat-lens-saver", 'en');
  return { 
    title: seoData.meta?.title || "Snapchat Lens Saver – Download & Share AR Lenses",
    description: seoData.meta?.description || "Save and archive popular Snapchat AR lenses and filters. Discover and keep unique Snapchat filter links online for free.",
    alternates: getSeoAlternates("snapchat-lens-saver", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("snapchat-lens-saver", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip Snapchat Lens Saver",
          description: "Extract, save, and share permanent links for Snapchat AR Lenses and Filters.",
          ratingValue: "4.7",
          reviewCount: "1800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Lens Saver", item: `${locale === 'en' ? '' : '/' + locale}/snapchat-lens-saver` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Lens Saver", href: "/snapchat-lens-saver" }
        ]} 
      />
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {[
              ...
              { title: "Map Downloader", href: "/snapchat-map-downloader" }
            ].map((tool, i) => (
              <Link 
                key={i}
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <SnapchatPreviewMockup type="lens" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-yellow-400 text-black px-4">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-xl font-bold mb-12 opacity-90">
                Extract permanent links and share your favorite AR Lenses easily with our secure tool.
            </p>
            <Link 
                href={`${locale === 'en' ? '' : '/' + locale}/snapchat-lens-saver`}
