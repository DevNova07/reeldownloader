...
import { YouTubeShortsCTA } from "./YouTubeShortsCTA"
...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const seoData = await getToolSeoData("youtube-shorts-downloader", 'en');
  return { 
    title: seoData.meta?.title || "YouTube Shorts Downloader – Save YT Shorts No Watermark HD",
    description: seoData.meta?.description || "Download YouTube Shorts in high definition without watermark. Save viral short clips directly to your storage layout.",
    alternates: getSeoAlternates("youtube-shorts-downloader", 'en'),
  }
}
export default async function Page(props: { params: Promise<{ locale: string }> }) {
  const params = await props.params;
  const locale = 'en';
  const seoData = await getToolSeoData("youtube-shorts-downloader", locale);
  ...
  return (
    <>
      <StructuredData
        ...
        data={{
          title: "SavClip YouTube Shorts Downloader",
          description: "Download YouTube shorts and reels in HD quality without watermarks for free.",
          ratingValue: "4.9",
          reviewCount: "15800"
        }}
      />
      ...
      <StructuredData
        ...
        data={[
          ...
          { name: "Shorts Downloader", item: `/youtube-shorts-downloader` }
        ]}
      />
      ...
      <Breadcrumb 
        ...
        items={[
          ...
          { label: "Shorts Downloader", href: "/youtube-shorts-downloader" }
        ]} 
      />
      ...
      <FeaturesSection platform="YouTube Shorts" />
      ...
      <FAQSection platform="YouTube Shorts" items={faqs} />
      ...
      <div className="bg-white dark:bg-black pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <div className="rounded-4xl overflow-hidden shadow-2xl border border-neutral-100 dark:border-neutral-800">
                <YoutubePreviewMockup type="shorts" />
              </div>
        </div>
      </div>
      ...
      <section className="py-20 bg-neutral-50 dark:bg-neutral-900/50 px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "How to Save YT Shorts in HD", desc: "Tips for getting the best quality for your short-form clips." },
              { title: "Trending Shorts Challenges 2026", desc: "What's going viral on YouTube Shorts right now." },
              { title: "Repurposing YT Shorts for TikTok", desc: "How to move your content without quality loss." }
            ].map((blog, i) => (
              <div key={i} className="group p-8 bg-white dark:bg-neutral-900 rounded-3xl shadow-lg border border-neutral-100 dark:border-neutral-800 hover:border-orange-500/30 transition-all">
                ...
                <Link href={`/`} className="text-xs font-black uppercase tracking-widest text-orange-600 hover:translate-x-1 transition-transform inline-block">Read More →</Link>
              </div>
            ))}
          </div>
        </div>
