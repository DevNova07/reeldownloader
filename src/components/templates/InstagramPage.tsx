...
import { Camera, PlaySquare, Film, StopCircle, Zap, ShieldCheck, CheckCircle2, HelpCircle, Info, Music as MusicIcon, ChevronRight, Layers, Compass, Monitor, Video, Download } from "lucide-react";
    ...
    <div className="flex flex-col">
      ...
      <StructuredData 
        ...
        data={[
          ...
          { name: "Instagram", item: `https://savclip.com/${locale}/instagram-video-downloader` },
      ...
      <section className={`relative overflow-hidden bg-linear-to-br from-fuchsia-600 via-purple-600 to-sky-500 px-4 pt-10 pb-10 sm:pt-16 sm:pb-24 sm:px-6 lg:px-8 shadow-[inset_0_-20px_60px_rgba(0,0,0,0.1)]`}>
        ...
        <div className="relative z-10 mx-auto max-w-7xl text-center flex flex-col items-center gap-3 sm:gap-6">
          ...
          <p className="mx-auto max-w-2xl text-base sm:text-lg font-medium text-white/80 tracking-tight italic drop-shadow-[0_4px_12px_rgba(0,0,0,0.3)] px-4 mt-2 leading-relaxed">
      ...
      <RelatedTools currentPlatform="instagram" /><InternalToolLinks currentPlatform="instagram" dict={dict} accentColor={cx.text} />

      

        
      
      <section className="py-16 bg-neutral-50 dark:bg-neutral-900 px-4 border-t border-neutral-100 dark:border-neutral-800">
        <div className="max-w-6xl mx-auto">
          <h2 className="mb-10 text-center text-2xl font-black text-neutral-900 dark:text-white tracking-widest uppercase italic">Latest Guides & Tips</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { title: "How to Download Videos on iPhone & Android", href: "/#guide" },
              { title: "Best Viral Hashtags to Use Today", href: "/hashtags" },
              { title: "Instagram Video Download Guide", href: "/instagram-video-downloader" }
            ].map((blog, idx) => (
              <Link key={idx} href={`/${locale}${blog.href.startsWith('/') ? blog.href.substring(1) : blog.href}`} className="p-6 rounded-3xl bg-white dark:bg-black border border-neutral-200 dark:border-neutral-800 hover:border-pink-500 transition-colors shadow-sm hover:shadow-xl group">
                <h3 className="font-bold text-neutral-900 dark:text-white group-hover:text-pink-500 transition-colors">{blog.title}</h3>
                <p className="mt-2 text-sm text-neutral-500 font-semibold uppercase tracking-wider">Read Guide →</p>
              </Link>
            ))}
          </div>
        </div>
